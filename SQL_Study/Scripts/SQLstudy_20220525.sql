 ...
 *
 * 별칭 부여 : SALARY -> 급여, 급여와 보너스 합친 금액 -> 총 급여, HIRE_DATE -> 입사일
 * 		   , FISRT_NAME와 LAST_NAME -> 이름
 */
...
--     LENGTH(FIRST_NAME) = 5
   AND (EXTRACT(YEAR FROM HIRE_DATE) >= 1998 AND EXTRACT(YEAR FROM HIRE_DATE) <= 2000);
...
SELECT FIRST_NAME || ' ' || LAST_NAME 이름
	 , SALARY 급여
	 , DECODE(JOB_ID, 'ST_CLERK', SALARY+200, 
			  'SA_MAN', SALARY+180, 'SA_REP', SALARY+150, 'IT_PROG', SALARY+100, SALARY) "인상된 급여"
  FROM EMPLOYEES;