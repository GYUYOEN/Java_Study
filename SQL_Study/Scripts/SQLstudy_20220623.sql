 ...
 * 		2. 한 단어는 최대 8자리까지
 * 		3. 약어의 조합으로 구성
 * 		4. 접미사 사용 - 컬렴명 성격 나타냄(complete_flag : boolean 유형의 컬럼, issue_date : 날짜 유형의 컬럼)
 * 		5. 항상 단수 사용
 * 		6. 항상 소문자 사용
 */
...
CREATE TABLE bread_t (
	 ...
	 , bread_type  VARCHAR2(20)  CONSTRAINT CK_BREAD_T_BREAD_TYPE CHECK(bread_type IN ('식빵', '일반빵', '크림빵', '조리빵', '패스츄리빵', '도넛빵', '유럽건강빵'))
	 , bread_price NUMBER 		 DEFAULT(0)
);

CREATE TABLE date_manager_t (
	   bread_id    NUMBER 		 CONSTRAINT PK_BREAD_REF_BREAD_NO PRIMARY KEY
	 , dm_proddate DATE   		 DEFAULT(SYSDATE)
	 , dm_note 	   VARCHAR2(100)
	 , CONSTRAINT FK_BREAD_REF_BREAD_NO FOREIGN KEY(bread_id) REFERENCES bread_t(bread_id)
);
...
SELECT * FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'DATE_MANAGER_T';
SELECT * FROM USER_COL_COMMENTS WHERE TABLE_NAME = 'DATE_MANAGER_T';
...
INSERT INTO bread_t VALUES(3, '피자빵',    10, '조리빵',    1800);
INSERT INTO bread_t VALUES(4, '앙버터라우겐', 5, '유럽건강빵', 2300);
INSERT INTO bread_t VALUES(5, '옛날꽈배기', 20, '도넛빵',    1200);
...
INSERT INTO date_manager_t VALUES(1, TO_DATE(20220625), '');
INSERT INTO date_manager_t VALUES(2, TO_DATE(20220626), '');
INSERT INTO date_manager_t VALUES(3, TO_DATE(20220624), '');
INSERT INTO date_manager_t VALUES(4, TO_DATE(20220627), '');
INSERT INTO date_manager_t VALUES(5, TO_DATE(20220628), '');
INSERT INTO date_manager_t VALUES(6, TO_DATE(20220623), '');
INSERT INTO date_manager_t VALUES(7, TO_DATE(20220629), '');
 ...
 *   - 컬럼명: student_t, 자료형: NUMBER DEFAULT(0), 컬럼주석: '과목점수평균'
 * 
 ...
 * 	  (주석으로 답을 작성하도록 한다.)
 */
ALTER TABLE student_t ADD stu_gender VARCHAR2(1 CHAR);
ALTER TABLE student_t ADD CONSTRAINT CK_STUDENT_T_STU_GENDER CHECK (stu_gender IN ('남', '여'));
...
ALTER TABLE score_t DROP COLUMN student_t;

ALTER TABLE score_t ADD student_avg NUMBER DEFAULT(0);
COMMENT ON COLUMN score_t.student_avg IS '과목점수평균';
...
-- TRUNCATE 테이블명; : ROLLBACK 해도 복원이 안됨, DELETE하면 COMMIT하기 전까지는 ROLLBACK 가능
-- 이유 : 자동 commit을 하기 때문
/*
 * 4. 답: TRUNCATE
...
COMMENT ON TABLE level_tb IS '학년';
COMMENT ON COLUMN level_tb.stu_id IS '학번';
COMMENT ON COLUMN level_tb.level_no IS '학년';
COMMENT ON COLUMN level_tb.class_no IS '반';
 
CREATE TABLE club_tb(
	   club_id 	      NUMBER       CONSTRAINT PK_CLUB_TB_CLUB_ID   PRIMARY KEY
	 , club_name      VARCHAR2(30) CONSTRAINT UK_CLUB_TB_CLUB_NAME UNIQUE
	 , club_president VARCHAR2(10)
	 , club_total     NUMBER
	 , club_teacher   VARCHAR2(10)
);
COMMENT ON COLUMN club_tb.club_id IS '동아리id';
COMMENT ON COLUMN club_tb.club_name IS '동아리명';
COMMENT ON COLUMN club_tb.club_president IS '회장';
COMMENT ON COLUMN club_tb.club_total IS '총인원';
COMMENT ON COLUMN club_tb.club_teacher IS '담임선생님';

CREATE TABLE student_tb (
	   stu_id NUMBER CONSTRAINT PK_STUDENT_TB_STU_ID PRIMARY KEY
	 , stu_name VARCHAR2(10)
	 , stu_age NUMBER CONSTRAINT CK_STUDENT_TB_STU_NAME CHECK (stu_age IN (17, 18, 19))
...
COMMENT ON COLUMN student_tb.stu_id IS '학번'; 
COMMENT ON COLUMN student_tb.stu_name IS '학생명'; 
COMMENT ON COLUMN student_tb.stu_age IS '나이'; 
COMMENT ON COLUMN student_tb.stu_phone IS '전화번호'; 
COMMENT ON COLUMN student_tb.stu_address IS '주소'; 
...
INSERT INTO student_tb VALUES (202206, '한소희', 17, ' 010-6233-697', '경기도 구리시',  1);
...
ALTER TABLE MART_T RENAME CONSTRAINT SYS_C0010773 TO CK_MART_T_POSSIBLE;
ALTER TABLE MART_T RENAME CONSTRAINT SYS_C0010776 TO FK_MART_T_BARCODE;
